...
export function ProjectAuditPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .listAudit({ projectId: id, pageSize: 100 })
  ...
  return (
    <Stack spacing={2.5}>
      ...
      <SectionCard flush>
        {loading ? (
          <LoadingState variant="skeleton" rows={5} height={56} />
        ) : logs.length === 0 ? (
          <EmptyState title="暂无审计记录" description="项目推进过程中产生的操作将自动留痕于此" icon={<HistoryOutlinedIcon />} />
        ) : (
                  ...
                  <Box sx={{ pb: isLast ? 0 : 2.5, minWidth: 0, flex: 1 }}>
                    <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap>
                      ...
                      <Chip
                        size="small"
                        variant="outlined"
                        label={`${AUDIT_ENTITY_LABEL[log.entityType] ?? log.entityType} · ${log.summary}`}
                        sx={{ height: 20, maxWidth: 360 }}
                      />
                      ...
                      <Stack spacing={0.5} sx={{ mt: 0.75, pl: 1, borderLeft: `2px solid ${tokens.border.subtle}` }}>
                        {log.diff.map((d) => (
                          <Typography key={d.field} variant="caption" color="text.secondary" sx={{ display: 'block' }}>
                            ...
                            ：{d.before || '—'} → <Box component="span" sx={{ color }}>{d.after || '—'}</Box>
                          </Typography>
                        ))}
                      </Stack>
                    )}
