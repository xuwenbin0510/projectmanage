...
async function main() {
  ...
  console.log('\n⑪ 降级桩（切真后端首屏不炸）');
  const listStubs = [
    /* wbs / board 批次 3 起是**真路由**，此处只保留「data 形态不炸」的兜底断言 */
    ['/api/projects/' + projectId + '/wbs', 'wbs list（B3 真路由）'],
  ...
  const board = okData(await call('GET', '/api/projects/' + projectId + '/board'), 'board（B3 真路由）');
  assert(Array.isArray(board && board.columns) && board.columns.length === 4, 'board → 四列');
  assert(!!(board && board.config && board.config.wipLimits), 'board → config.wipLimits 存在');
  assertEq(board && board.config && board.config.wipLimits['进行中'], 5, 'board → 默认 WIP 上限「进行中」= 5');
  ...
  /* B3 后 POST /wbs 是真路由：缺 owner / estimateDays 必须命中叶子完整性校验，而不是 501 */
  const leafIncomplete = await call('POST', '/api/projects/' + projectId + '/wbs', {
    parentId: null,
    nodeType: 'task',
    name: 'smoke-incomplete',
  });
  expectError(leafIncomplete, 'E_WBS_LEAF_INCOMPLETE', 400, 'POST /wbs 缺 owner/estimateDays → 叶子完整性校验');

  /* 「未实现写操作」样本改用 PATCH /projects/:id（批次 4 才开放） */
  const notImpl = await call('PATCH', '/api/projects/' + projectId, { name: 'x' });
  expectError(notImpl, 'E_NOT_IMPLEMENTED', 501, '未实现写操作（PATCH /projects/:id 留待批次 4）');
