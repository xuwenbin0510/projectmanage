...
> **P2 实施记录（B13.5）**：上述四项已全部补齐，改动仅限 `OverdueTaskDrawer.tsx` + `types/dashboard.ts`（新增 `ownerId`）+ `MetricsPage.tsx` / `WorkbenchPage.tsx`（传入 `currentUserId`）。具体：
> - **P2-1 行点击跳 WBS 页**：`DataTable` 加 `onRowClick` → `navigate(ROUTES.projectWbs(projectId))`。
> - **P2-2 分页**：任务数 > 8 时启用 `DataTable` 内置 `pagination`（零新依赖，前端切片）。
> - **P2-3 二次筛选**：状态下拉（取当前 Tab 实际出现状态）+ 关键字搜索（任务名/负责人），在已逾期/临期集合内再过滤。
> - **P2-4 仅看我负责**：传入 `currentUserId` 时显示开关，仅保留 `owner === 当前用户` 的任务；未登录则不显示。
> 验证：`tsc --noEmit` 0 错误、`vite build` 0 错误（1983 模块）。
