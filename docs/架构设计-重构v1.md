...
| D5 | **里程碑单向规则：提前直接改 / 延后走变更** | 直接 UPDATE 会绕过约束 | `planned_date` **提前可直接改**（`delayDays` 可为负，写审计「日期提前」）；**延后**只允许变更单驱动写入——直接 PUT 延后→`E_MS_NEED_CHANGE` 并回传预填变更单草稿。**「提前被拒」路径已废弃**（WBS 重构后对齐实现，旧死码删除） |
...
| 里程碑 `planned_date` 提前 | `milestone.service.assertDateRule()` | 直接生效（`delayDays` 可为负）+ 审计「日期提前」；**无「提前被拒」路径** |
...
| **T27** | 里程碑服务：单向约束 + 基线 + 锚点 | P0-05 | T25 | BE | P0 | `server/services/milestone.service.js`、`server/routes/milestones.routes.js` | **提前 → 直接生效**（`delayDays` 可为负，审计「日期提前」）；延后 → `E_MS_NEED_CHANGE` + changeDraft；`baseline_date` 任何请求都改不动；仅 `applyDateChange()` 可写 `planned_date`；`stageId`/`anchor` 补锚可改、跨项目拒 |
...
| `E_STAGE_SEQUENCE` | 400 | 不可跳阶（WBS 重构 D-5：本轮契约保留、实现未落地） | P0-02 |
| `E_MS_NEED_CHANGE` | 409 | 延后须走变更（`data.changeDraft`）；**提前无拦截码，直接生效** | P0-05 |
| `E_WBS_LEAF_INCOMPLETE` | 400 | 叶子节点缺负责人或估算（R-5） | P0-06 |
| `E_WBS_CYCLE` | 400 | 移动会造成环 | P0-06 |
| `E_WBS_PARENT_TYPE` | 400 | 子类型不在父节点白名单（R-2） | P0-06 |
| `E_WBS_DEPTH` | 400 | 超过层级上限（R-1，含 move 子树整体判定） | P0-06 |
| `E_WBS_STAGE_UNBOUND` | 400 | stage 未绑生命周期阶段（R-6） | P0-06 |
| `E_WBS_TYPE_LOCKED` | 400 | 有子节点改 nodeType（R-4） | P0-06 |
