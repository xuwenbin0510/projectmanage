...
>
> **2026-09-14 补充（落地后校对，必读）**：A2 把 capability 判定放在最前，故「责任角色」**是兜底通道而非排他约束**——持有能力/override 的角色可操作任意门与任意项（线上 `gate:item:check` 授予 11 个角色、`gate:decide` 授予 7 个）。用户拍板**保留该宽松语义**，因此报错与界面文案已同步改为事实陈述（不再宣称「仅责任角色可操作」）：后端 `rbac.assertGateOwnerOrOverride` 403 文案、mock 同源文案、`ProjectOverviewPage` 不可提交 Tooltip 均为「你当前角色无此权限（责任角色：X）」，检查项在非责任人操作时标注「（由你代操作）」。下文 §5.1 / §5.4 中的旧报错文案仅作设计历史留存。
